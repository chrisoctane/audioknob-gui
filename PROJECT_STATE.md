...
- **Kernel RT extras (Main + Dev)** - kernel cmdline knobs for preempt=full, clocksource=tsc, tsc=reliable, nmi_watchdog=0, nosoftlockup, nosmt (TSC knobs in Main under Advanced lock; remaining extras in Dev)
...
- Development: set `AUDIOKNOB_DEV_REPO=/path/to/repo` environment variable
  ...
  - Privileged repo runs must also install the polkit wrapper via `sudo ./packaging/install-polkit.sh`, which writes `/etc/audioknob-gui/dev.conf` so the fixed-path root worker imports the same checkout
  - GUI-side worker helpers refuse privileged dev-mode actions if `AUDIOKNOB_DEV_REPO` and `/etc/audioknob-gui/dev.conf` do not point at the same repo, to avoid split-brain behavior between repo GUI code and an older installed root worker
...
Privileged knob/system actions still require `sudo ./packaging/install-polkit.sh` so `/usr/libexec/audioknob-gui-worker` reads the same repo path from `/etc/audioknob-gui/dev.conf`.
