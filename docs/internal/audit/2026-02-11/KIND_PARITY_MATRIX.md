...
| `irq_affinity` | 1 | `irq_pinning` | Full | Pass | Pass | Pass | Pass | Pass | Pass | Preview/apply/status present; force reset now restores writable IRQ masks to kernel defaults and removes audioknob IRQ persistence artifacts. |
...
| `power_profile` | 1 | `power_profile_performance` | Full | Pass | Pass | Pass | Pass | Limited | Pass | Backend-aware preview/apply/status; force reset uses conservative `balanced` target with backend verification. |
| `qjackctl_server_prefix` | 1 | `qjackctl_server_prefix_rt` | Full | Pass | Pass | Pass | Pass | Pass | Pass | User-scope preview/apply/status and transaction restore path; force reset strips RT/taskset + clears audioknob post-start hook. |
...
| `wpctl_profile` | 1 | `pipewire_pro_audio_profile` | Full | Pass | Pass | Pass | Pass | Limited | Limited | Preview/apply/status present; force reset branch is explicit safe-decline when deterministic fallback profile inference is unsafe. |
